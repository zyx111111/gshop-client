...
<script>
    ...
    export default {
        ...
        mounted() {
          if(!this.info.pics){     //如果是直接访问本页面，这个info还没拿到，要监视加nextTick
            return
          }
          this._initScroll()
        },
        methods: {
          _initScroll() {
            new BScroll('.shop-info')   //如果不是直接访问本页面，而是从别的两个路由切换过来
            // 可以不用this.nextTick和监视，因为info这个数据是从shop组件就已经拿到了，当在mounted中创建这个BScroll对象的时候，数据和页面都是得到数据然后显示的
            // 动态计算ul的宽度,因为ul的宽度没有包住li，所以不能形成滚动，也可以直接修改样式让它包裹住
